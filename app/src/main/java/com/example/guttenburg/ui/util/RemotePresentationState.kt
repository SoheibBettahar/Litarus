...
fun Flow<CombinedLoadStates>.asRemotePresentationState(): Flow<RemotePresentationState> =
    scan(RemotePresentationState.INITIAL) { state, loadState ->
        when (state) {
            //initial/success/error state and about to load data from network -> REMOTE_LOADING
            RemotePresentationState.INITIAL, RemotePresentationState.PRESENTED -> when (loadState.mediator?.refresh) {
                is LoadState.Loading -> RemotePresentationState.REMOTE_LOADING
                else -> state
            }
            //REMOTE_LOADING state and about to load data from database -> SOURCE_LOADING
            ...
            //SOURCE_LOADING state and about to load data from database -> PRESENTED
